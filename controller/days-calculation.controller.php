...
switch ($url[5]) {
    ...
    case 'getall':
        ...
        if (TokenTool::isValid($token)){
            ...
            $query = "SELECT dc.IdDayCalculation, c.CompanyName, c.CompanyLogo, co.ContactName, co.ContactPhone, co.ContactEmail, co.ContactCharge, co.ContactPhoto, s.ServiceShortName, sec.IAF_MD5, sec.SectorCluster, sec.SectorCategory, sec.SectorSubcategory, sec.SectorRiskLevel, app.AppLanguage, dc.DayCalculationDate, dc.DaysCalculationStatus, dc.DaysInitialStage, dc.DaysSurveillance, dc.DaysRR FROM applications AS app JOIN companies AS c ON app.IdCompany = c.IdCompany JOIN contacts AS co on app.IdContact = co.IdContact JOIN services as s ON app.IdService = s.IdService JOIN sectors As sec ON app.IdSector = sec.IdSector JOIN days_calculation AS dc ON dc.IdApp = app.IdApp";
    ...
    case 'company':
        ...
        if (TokenTool::isValid($token)){
            $query = "SELECT dc.IdDayCalculation, c.CompanyName, c.CompanyLogo, co.ContactName, co.ContactPhone, co.ContactEmail, co.ContactCharge, co.ContactPhoto, s.ServiceShortName, sec.IAF_MD5, sec.SectorCluster, sec.SectorCategory, sec.SectorSubcategory, sec.SectorRiskLevel, app.AppLanguage, dc.DayCalculationDate, dc.DaysCalculationStatus, dc.DaysInitialStage, dc.DaysSurveillance, dc.DaysRR FROM applications AS app JOIN companies AS c ON app.IdCompany = c.IdCompany JOIN contacts AS co on app.IdContact = co.IdContact JOIN services as s ON app.IdService = s.IdService JOIN sectors As sec ON app.IdSector = sec.IdSector JOIN days_calculation AS dc ON dc.IdApp = app.IdApp  WHERE app.IdCompany = :idCompany ORDER BY dc.DayCalculationDate DESC;";
    ...
    case 'get':
        ...
        if (TokenTool::isValid($token)){
            $query = "SELECT * FROM days_calculation WHERE IdDayCalculation = :idDayCalculation;";
