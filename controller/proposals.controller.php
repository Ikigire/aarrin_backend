...
switch ($url[5]) {
    ...
    case 'getall':
        ...
        if (TokenTool::isValid($token)){
            ...
            $query = "SELECT c.CompanyName, c.CompanyLogo, co.ContactName, co.ContactPhone, co.ContactEmail, co.ContactCharge, co.ContactPhoto, s.ServiceShortName, sec.IAF_MD5, sec.SectorCluster, sec.SectorCategory, sec.SectorSubcategory, sec.SectorRiskLevel, pr.IdProposal, pr.ProposalStatus, pr.ProposalDate, pr.ProposalExpirationDate, pr.CurrencyType, pr.TotalInvestment, pr.ProposalApproved, pr.ProposalClientApproved, emp1.EmployeeName + ' ' + emp1.EmployeeLastName AS 'EmployeeCreator', emp1.EmployeePhoto AS 'EmployeeCreatorPhoto', emp2.EmployeeName + ' ' + emp2.EmployeeLastName AS 'EmployeeReviewer', emp2.EmployeePhoto AS 'EmployeeReviewerPhoto' FROM applications AS app JOIN companies AS c ON app.IdCompany = c.IdCompany JOIN contacts AS co on app.IdContact = co.IdContact JOIN services as s ON app.IdService = s.IdService JOIN sectors As sec ON app.IdSector = sec.IdSector JOIN days_calculation AS dc ON dc.IdApp = app.IdApp JOIN proposals AS pr ON pr.IdDayCalculation = dc.IdDayCalculation JOIN personal AS emp1 ON pr.IdProposalCreator = emp1.IdEmployee JOIN personal AS emp2 ON pr.IdProposalReviewer = emp2.IdEmployee";
    ...
    case 'company':
        ...
        if (TokenTool::isValid($token)){
            $query = "SELECT c.CompanyName, c.CompanyLogo, co.ContactName, co.ContactPhone, co.ContactEmail, co.ContactCharge, co.ContactPhoto, s.ServiceShortName, sec.IAF_MD5, sec.SectorCluster, sec.SectorCategory, sec.SectorSubcategory, sec.SectorRiskLevel, pr.IdProposal, pr.ProposalStatus, pr.ProposalDate, pr.ProposalExpirationDate, pr.CurrencyType, pr.TotalInvestment, pr.ProposalApproved, pr.ProposalClientApproved, emp1.EmployeeName + ' ' + emp1.EmployeeLastName AS 'EmployeeCreator', emp1.EmployeePhoto AS 'EmployeeCreatorPhoto', emp2.EmployeeName + ' ' + emp2.EmployeeLastName AS 'EmployeeReviewer', emp2.EmployeePhoto AS 'EmployeeReviewerPhoto' FROM applications AS app JOIN companies AS c ON app.IdCompany = c.IdCompany JOIN contacts AS co on app.IdContact = co.IdContact JOIN services as s ON app.IdService = s.IdService JOIN sectors As sec ON app.IdSector = sec.IdSector JOIN days_calculation AS dc ON dc.IdApp = app.IdApp JOIN proposals AS pr ON pr.IdDayCalculation = dc.IdDayCalculation JOIN personal AS emp1 ON pr.IdProposalCreator = emp1.IdEmployee JOIN personal AS emp2 ON pr.IdProposalReviewer = emp2.IdEmployee WHERE app.IdCompany = :idCompany ORDER BY dc.DayCalculationDate DESC;";
